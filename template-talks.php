...
/**
 * Template Name: Talks
 * Template for listing all Talks.
 *
...
<header class="writing__header">
    <h1 class="page-title"><?php echo esc_html($page_title ?: __('Talks', 'fabian-theme')); ?></h1>
    <?php if ($page_intro): ?>
        <div class="writing__intro talks__intro"><?php echo apply_filters('the_content', $page_intro); ?></div>
    <?php else: ?>
        <p class="writing__intro"><?php _e('A living archive of talks, workshops, and ongoing series.', 'fabian-theme'); ?></p>
    <?php endif; ?>
</header>

<?php if ($talks_query->have_posts()):
    $talk_count = (int) $talks_query->found_posts;
    ...
    <section class="talks">
        <div class="talks-grid <?php echo esc_attr($grid_mod); ?>">
            <?php while ($talks_query->have_posts()):
