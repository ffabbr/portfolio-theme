 ...
 * Template for the Writing/Blog archive page
 *
    ...
    <?php if ( $posts_query->have_posts() ) : ?>
        ...
        <?php if ( $total_pages > 1 ) : ?>
            <nav class="pagination" aria-label="<?php esc_attr_e( 'Pagination', 'fabian-theme' ); ?>">
                <?php if ( $paged > 1 ) : ?>
                    <a href="<?php echo esc_url( ( $paged === 2 ? home_url( '/writing/' ) : home_url( '/writing/page/' . ( $paged - 1 ) . '/' ) ) . '#writing-posts' ); ?>" class="pagination__prev">
                ...
                <ul class="pagination__list">
                    <?php for ( $i = 1; $i <= $total_pages; $i++ ) : ?>
                        <li>
                            ...
                            <?php else : ?>
                                <a href="<?php echo esc_url( ( $i === 1 ? home_url( '/writing/' ) : home_url( '/writing/page/' . $i . '/' ) ) . '#writing-posts' ); ?>" class="pagination__page">
                                    <?php echo esc_html( $i ); ?>
                                </a>
                            <?php endif; ?>
                        </li>
                ...
                <?php if ( $paged < $total_pages ) : ?>
                    <span class="pagination__separator" aria-hidden="true"></span>
                    <a href="<?php echo esc_url( home_url( '/writing/page/' . ( $paged + 1 ) . '/' ) . '#writing-posts' ); ?>" class="pagination__next">
