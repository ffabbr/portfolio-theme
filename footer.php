...
async function loadMembers() {
    // Load the official embed.js file
    const res = await fetch("https://polyring.ch/embed.js");
    const js = await res.text();

    // Extract the MEMBERS array definition
    const match = js.match(/MEMBERS\s*=\s*(\[.*\]);/s);
    if (!match) throw new Error("Could not find MEMBERS array in embed.js");

    // Parse MEMBERS into a JS array
    return eval(match[1]); // safe-ish here because we control the source
}
